...
## Krótka historia
Mogłoby się wydawać, że chmury obliczeniowe to wynalazek ostatnich kilku lat. A jednak idea chmur obliczeniowych
sięga końcówki lat 60-tych ubiegłego wieku i zakłada istnienie rozproszonych, skalowalnych systemów, dostępnych
z dowolnego miejsca na świecie. Przez lata jednak niewiele się działo w tym kierunku, przynajmniej w sferze rozwiązań
...
dołączył Google a chwilę później Microsoft. Następnie nastąpił boom na różne usługi dostępne w modelu chmury obliczeniowej i 
ten boom trwa po dzień dzisiejszy.
...
pewne mechanizmy odciążające użytkowników z instalacji oprogramowania, konfiguracji serwerów itd. Klient musi jedynie
wgrać swoją aplikację oraz wskazać plaftormie sposób jej uruchomienia. *SaaS* jest natomiast niczym innym jak 
oprogramowaniem dostępnym przez przeglądarkę, a niekiedy też natywnym aplikacjom. Oprogramowanie tego typu jest bardzo
zróżnicowane - mogą to być pakiety biurowe, programy graficzne albo księgowe.

Koszty usług chmurowych są bardzo zróżnicowane i każdy operator ma swój cennik za poszczególne oferowane produkty.
...
dla platformy. Sprawdźmy najpierw jak dokladnie działają obydwa te serwisy.

### Amazon EC2
Usługa Elastic Cloud Computing jest niczym innym, jak udostępnianiem wirtualnych maszyn. Możemy zatem utworzyć maszynę
posiadającą określonej wielkości moc obliczeniową i dysk twardy. Dodatkowo wybieramy system operacyjny, jaki ma na niej
zostać zainstalowany oraz generujemy parę kluczy RSA, potrzebnych do połaczenia się z maszyną przez SSH. Jeśli chcemy 
wykorzystać swój zestaw kluczy możemy pominąć generację pary kluczy a zamiast tego podać swój klucz publiczny. W obecnej
chwili możemy wybierać między np. Ubuntu, Debianem, Centosem, wieloma innymi UNIXowymi oraz Microsoft Server. Każda
...
### Heroku
Heroku jest platformą uruchomieniową dla aplikacji napisanych w różnych językach - Ruby,Pythona, Java, NodeJS i innych.
...
istnieć wiele wyizolowanych kontenerów. Aby uruchomić aplikację na Heroku, należy poinformować platformę jak należy to 
...
restartu będzie bezpowrotnie utracone. Jest to bardzo ważne ograniczenie, które należy mieć w świadomości już na etapie
...
* usypianie dyno - jeśli nasza aplikacja wykorzustuje tylko jedno Dyno i w ciagu godziny nie było wykonane żadne zapytanie,
Dyno zostaje uśpione. Uśpione dyno nie pracuje, więc jeśli zostanie wysłane do niego zapytanie to Dyno Manager musi je 
najpierw wybudzić, co powoduje, że pierwsze zapytania mogą mieć dłuższy czas oczekiwania na odpowiedź.

Niewątpliwym plusem Heroku jest natomiast minimalna ilość pracy jaką należy wykonać, aby uruchomić aplikację. Dodatkowo 
Heroku oferuje bardzo dużo dodatków, np.
...
jak i HTTPS. Tworząc aplikację dostajemy również repozytorium Git, służące do wgrywania kolejnych wersji aplikacji.
...
Amazonu mamy do wyboru dwie opcje. Pierwszą jest instalacja serwera bazodanowego na tej samej maszynie, na której
...
Na Heroku sprawa jest rozwiązana za nas. Baza danych dostępna jest jako dodatek a my musimy jedynie wybrać plan cenowy. 
Na początek wystarczy plan darmowy, ale posiada limit 10 000 wierszy. Po przekroczeniu tej ilości baza przestanie 
dodawać nowe rekordy i będziemy zmuszeni do zmiany planu na płatny, a te różnią sie ilością pamięci podręcznej.

### Wgrywanie plików do 30 MB
To jest kluczowy punkt w specyfikacji wymagań. Umożliwienie wgrania plików przez użytkowników oznacza, że musimy te 
pliki gdzieś przechowywać, a ich rozmiar będzie rósł wraz z ilością użytkowników. Jeśli pamiętamy ograniczenia zarówno 
...
skonfigurować. Na Heroku niestety, chcąc korzystać z HTTPS z własną domeną, poza wgraniem certyfikatów jesteśmy zmuszeni
do dokupienia dodatku *SSL Endpoint* w cenie 20$/miesiąc.

### Skalowalność
Skalowalność jest bardzo ważym aspektem aplikacji i obywa serwisy oferują wsparcie. Tak samo jak w przypadku innych 
wymagań zapewnienie skalowalności na EC2 sprawa jest bardziej złożone i wymaga dodania maszyny typu *Load Balancer* 
oraz utworzenia żądanej ilości maszyn z pracującą aplikacją. Na Heroku można zmienić ilość używanych Dynos korzystając 
z przeglądarki i ustawiając żądaną ilosć w konfiguracji aplikacji.
...
miesięcznych opłat, lecz będzie wymagało większych nakładów na konfiguracje i utrzymanie systemu. Heroku jest wygodniejsze
w obsłudze, jednak w środowisku produkcyjnym będzie na pewno droższe od EC2.

## Podsumowanie
Jak widać wybór serwisu chmurowego do serwowania naszej aplikacji nie jest sprawą prostą i wymaga bardzo indywidualnego 
podejścia. Pod uwagę należy wziać zarówno obecne wymagania funkcjonalne i pozafunkcjonalne oprogramowania, perspektywy 
rozwoju i budżet.
