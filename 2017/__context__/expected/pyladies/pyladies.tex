...
\section[warsztaty-pyladies-pycon-pl-2017]{Warsztaty PyLadies / PyCon PL
2017}

Witaj na warsztatach PyLadies! Podczas tego kursu nauczysz się
programować w języku {\bf Python} od podstaw. Aby zacząć nie
potrzebujesz nic instalować, nie jest wymagana żadna znajomość
programowania czy informatyki. Wystarczy przeglądarka internetowa i chęć
zdobywania wiedzy!

\subsection[czego-uczą-te-warsztaty]{Czego uczą te warsztaty}
...
zrozumiesz Pythona na poziomie, który pozwoli Ci pisać przydatne
narzędzia oraz zgłębiać bardziej zaawansowane aspekty tego języka.

Podsumowując: Twoja przygoda z programowaniem zaczyna się właśnie tutaj!

\subsection[legenda]{Legenda}

Zanim zaczniemy, zapoznaj się z oznaczeniami, które zostały użyte na
kolejnych stronach kursu.

$\sim$ oznacza miejsce, w którym dajemy Ci do wykonania jakieś zadanie.
Jeżeli instrukcje nie są jasne, lub zadanie sprawia Ci kłopoty - poproś
o pomoc mentora.

$\swarrow$ to podsumowanie rozdziału. Zanim przejdziesz dalej, upewnij
się, że rozumiesz wszystkie wymienione tam tematy. Jeśli masz
jakiekolwiek wątpliwości - zapytaj mentora. Nie sugeruj się innymi -
właśnie po to robimy podsumowanie, żeby upewnić się, że każdy zrozumiał
dany rozdział.

{\bf Pogrubionym tekstem} wyróżniliśmy istotne słowa i zwroty. Warto je
zapamiętać, aby łatwiej przyswoić kolejne rozdziały.

\type{W taki sposób} oznaczyliśmy przykładowy kod programów.

\subsection[mentorzy]{Mentorzy}

Nasz zespół mentorów jest tutaj dla Ciebie! Jeżeli masz pytania,
cokolwiek jest niejasne albo nie wiesz jak wykonać jakieś zadanie -
poproś kogoś z nas o pomoc. Chętnie odpowiemy, opowiemy i pomożemy Ci
zrozumieć każdy temat. Nie krępuj się: my równeż kiedyś~zadawaliśmy
pytania i właśnie dzięki temu sami możemy teraz na nie odpowiadać.

\subsection[spis-treści]{Spis treści}
...
\subsection[pushpin-podsumowanie]{$\swarrow$ Podsumowanie}
...
oraz cyfry. Istnieją tysiące znaków, jakich możesz użyć {[}1{]}.

$\sim$ Zanim przejdziesz dalej spróbuj samodzielnie utworzyć stringi z
...
$\sim$ Teraz wypróbuj te metody w taki sposób, żeby zobaczyć efekty ich
...
$\sim$ Zdefiniuj kilka stringów i na każdym z nich wywołaj każdą z
...
$\sim$ Sprawdź długość Twojego imienia i nazwiska. Zobacz jaką długość
ma pusty string, czyli \type{''}.

\subsection[pushpin-podsumowanie-1]{$\swarrow$ Podsumowanie}
...
$\sim$ Użyj funkcji \type{help} i przeczytaj dokumentację do metod
\type{replace} i \type{count} oraz do funkcji \type{len}.

\subsection[pushpin-podsumowanie-2]{$\swarrow$ Podsumowanie}
...
$\sim$ Spróbuj samodzielnie wykonać kilka działań arytmetycznych.
...
$\sim$ Czy wiesz kiedy Python zwróci {\em float} a kiedy {\em integer}?
...
\subsection[pushpin-podsumowanie-3]{$\swarrow$ Podsumowanie}
...
$\sim$ Wywołaj błędy i przeczytaj ze zrozumieniem wyjątki spowodowane
...
\subsection[pushpin-podsumowanie-4]{$\swarrow$ Podsumowanie}
...
$\sim$ Utwórz zmienne \type{imie} oraz \type{nazwisko}, przypisz do
...
$\sim$ Zobacz co się stanie, kiedy spróbujesz stworzyć zmienną, której
...
\subsection[pushpin-podsumowanie-5]{$\swarrow$ Podsumowanie}
...
$\sim$ Napisz funkcję \type{cena_brutto}, która przyjmuje argumenty
\type{cena_netto} oraz \type{vat} i zwraca wartość netto obliczoną
według wzoru \type{netto * (1 + vat)}.

$\sim$ Napisz funkcję \type{imie_nazwisko}, która przyjmuje argumenty
...
Poza tym mamy jeszcze 67 innych funkcji wbudowanych {[}2{]}. Część z
nich poznasz w kolejnych rozdziałach, a kilka innych opisaliśmy poniżej.
...
$\sim$ Zamień na string liczbę ujemną.

$\sim$ Zobacz co się stanie, jeżeli jako argument przekażesz do
\type{str} tekst.
...
$\sim$ Zobacz co się stanie, gdy przekażesz do funkcji \type{int}
liczbę z częścią ułamkową~(float), np. \type{3.14}.

$\sim$ Zobacz co się stanie, jeśli przekażesz do \type{int} stringa, w
którym nie ma żadnej liczby.

$\sim$ Zobacz co sie stanie, kiedy przekażesz do \type{int} stringa, w
...
$\sim$ Zobacz jak zachowa się funkcja \type{float}, gdy wywołasz ją z:
integerem, stringiem z samymi literami, stringiem z samymi cyframi.

\subsection[pushpin-podsumowanie-6]{$\swarrow$ Podsumowanie}
...
$\sim$ Napisz funkcję, która przyjmuje argument \type{rok_urodzenia},
wypisuje tekst \type{Masz X lat}, gdzie \type{X} to wiek w roku 2017,
oraz zwraca ten wiek.

\subsection[formatowanie-stringów]{Formatowanie stringów}

W tym miejscu warto wrócić do stringów i opowiedzieć o jeszcze jednej,
bardzo przydatnej metodzie: \type{format}. Służy ona do
{\bf formatowania stringów}, czyli \quotation{wstawiania} do nich
wartości zmiennych. Spójrz na poniższy przykład:
...
Jak widzisz, wywołanie metody \type{format} spowodowało, że para znaków
\mono{\{\}} została zastąpiona argumentami funkcji. W podobny sposób
możemy wstawić dowolną liczbę i typ obiektów:
...
Możliwości metody \type{format} nie kończą się na zwykłym wstawianiu
wartości do stringa.
\useURL[url1][https://docs.python.org/3.6/library/string.html\#formatspec][][Dokumentacja
Pythona]\from[url1] w szczegółach opisuje tę funkcję. Warto przyjrzeć
się choćby przykładom, które tam zamieszczono.

$\sim$ Zobacz co się stanie, jeżeli liczba argumentów metody
\type{format} będzie {\bf mniejsza} niż liczba wystąpień \mono{\{\}} w
stringu.

\subsection[pushpin-podsumowanie-7]{$\swarrow$ Podsumowanie}

W tym rozdziale:

\startitemize[packed]
\item
  poznaliśmy funkcję \type{print} oraz metodę \type{format}.
\stopitemize
...
$\sim$ Napisz funkcję \type{element}, która przymuje dwa argumenty,
listę oraz numer indeksu (integer) i zwraca element listy znajdujący się
pod podanym indeksem.

$\sim$ Napisz funkcję \type{ostatni_element}, która jako argument
...
$\sim$ Napisz funkcję, która jako argument przyjmuje listę i dodaje na
...
$\sim$ Napisz funkcję, która usuwa z listy dwa ostatnie elementy, po
...
$\sim$ Sprawdź co się stanie jeżeli spróbujemy usunąć element, którego
nie ma na liście.

$\sim$ Napisz funkcję, która przyjmuje dwa argumenty: listę oraz
...
$\sim$ Sprawdź co się stanie jeżeli spróbujemy pobrać indeks elementu,
...
$\sim$ Napisz funkcję, która jako argument przyjmuje listę i wypisuje
...
$\sim$ Napisz funkcję, która jako argument przyjmie listę, posortuje
ją, a następnie zwróci jej ostatni element. (W ten sposób otrzymamy
własną wersję funkcji \type{max}!)

\subsection[wycinki-list]{Wycinki list}
...
jest przygotowany na taką sytuację: umożliwia utworzenie {\em wycinka}
listy (ang. {\em slice}). Aby stworzyć wycinek, należy wpisać nazwę
listy, a następnie w nawiasach kwadratowych indeksy pierwszego i
ostatniego elementu wycinka, odzielone dwukropkiem.
...
końcowym (w tym przypadku: \type{5}) nie zostanie dołączony do wycinka.
...
Jeżeli indeks końcowy będzie liczbą ujemną, to pozycja ostatniego
elementu wycinka będzie liczona od końca listy:
...
$\sim$ Zobacz co się stanie, jeżeli indeks początkowy będzie liczbą
ujemną, lub jeżeli indeks końcowy będzie większy niż długość listy.

\subsection[pushpin-podsumowanie-8]{$\swarrow$ Podsumowanie}
...
można rozwiązać za pomocą {\bf pętli}, czyli instrukcji, która wykonuje
...
$\sim$ Napisz funkcję, która jako argument przyjmie listę liczb i
wpisze na ekran wartość każdej z nich podniesioną do kwadratu.

$\sim$ Napisz funkcję, który przyjmie listę stringów i zwróci nową
...
$\sim$ Napisz funkcję, która jako argument przyjmie string i wypisze
każdą jego literę wraz z liczbą wystąpień tej litery w stringu (użyj
metody \type{count}).

\section[metoda-split]{Metoda \type{split}}

Możemy również iterować po słowach. Służy do tego metoda \type{split}:
...
Tak naprawdę metoda \type{split} ma dużo szersze zastosowanie. Jeżeli
podamy jej jako argument jakiś znak, wtedy string zostanie podzielony w
miejscach występowania tego znaku:
...
$\sim$ Napisz funkcję, która jako argument przyjmuje string i wypisuje
wszystkie jego słowa, każde w osobnej linijce.

\subsection[range]{\type{range}}

W tym miejscu warto wspomnieć o funkcji wbudowanej \type{range}.
Przyjmuje ona jako argumenty dwa integery: początek i koniec przedziału
liczbowego, który zwraca. Możemy następnie iterować po takim przedziale
i wówczas elementami będą kolejne liczby całkowite:
...
Co ciekawe, możemy podać tylko jeden argument, który wtedy jest
traktowany jako koniec przedziału, zaś za początek przyjmuje się liczbę
0. Kolejny przykład pokazuje jak wypisać liczby od 0 do 99:
...
$\sim$ Napisz funkcję, która przyjmie jeden argument o nazwie
\type{limit} i zwróci listę wartości od 0 do \type{limit} podniesionych
do kwadratu.

\subsection[pushpin-podsumowanie-9]{$\swarrow$ Podsumowanie}
...
\item
  poznaliśmy funkcję wbudowaną \type{range}.
\stopitemize

\section[rozdział-11.-krotki]{Rozdział 11. Krotki}

W tym rozdziale:

\startitemize[packed]
\item
  dowiesz się czym jest {\bf krotka}.
\stopitemize

\subsection[krotka]{Krotka}
...
Powyższy przykład to definicja {\bf krotki}. Na pierwszy rzut oka różni
się ona od listy tylko na tym, że zamiast nawiasów kwadratowych ma
okrągłe. Jednak krotka posiada jedną istotną cechę, która odróżnia ją od
listy: nie można jej modyfikować. Oznacza to, że do raz utworzonej
krotki nie można dodawać elementów, usuwać ich, ani nawet zmieniać.
Dlatego też nie znajdziemy w krotce metod \type{append} ani
\type{remove}. Za to z powodzeniem możemy odnosić się do poszczególnych
elementów używając ich indeksów:
...
Oznacza to, że możemy również iterować po krotce pętlą \type{for}. Jeśli
zaś przekażemy krotkę do funkcji \type{len}, to dostaniemy liczbę jej
elementów.

$\sim$ Napisz funkcję, która jako argument przyjmie krotkę i zwraca
listę, która zawiera dokładnie takie same elementy.

\subsection[zastosowanie-krotek]{Zastosowanie krotek}

Z pozoru krotka może się wydawać czymś zupełnie zbędnym. Po co nam taka
lista, której nie można modyfikować? Otóż w wielu przypadkach
potrzebujemy dokładnie czegoś takiego. Dobrym przykładem jest właśnie
zbiór nazw walut. Wyobraźmy sobie, że piszemy program do przeliczania
wartości pieniężnych między różnymi walutami. W naszym programie będą
zmieniały się kursy, ale same waluty będą cały czas takie same. Dlatego
możemy je zdefiniować w krotce, tym samym chroniąc się przed modyfikacją
zbioru walut, co byłoby niepożądane. Nawet jeżeli w przyszłości będziemy
chcieli dodać kolejną walutę, łatwiej będzie wydać nową wersję programu
niż ryzykować modyfikację zbioru, który powinien pozostać niezmieniony.

Podobnych przykładów jest więcej i czytając kod różnych programów szybko
przekonasz się, że krotki przydają się częściej niż mogło by się
wydawać.

\subsection[pushpin-podsumowanie-10]{$\swarrow$ Podsumowanie}
...
\section[rozdział-12.-prawda-i-fałsz]{Rozdział 12. Prawda i fałsz}
...
o dalszym przebiegu naszego programu. W tym celu używamy {\bf instrukcji
warunkowej} \type{if}:
...
wcięciu, instrukcje, które zostaną wykonane jeżeli warunek będzie
prawdziwy (mówimy: jeżeli warunek zostanie spełniony).

$\sim$ Napisz funkcję, która przyjmuje argumenty \type{element} i
...
$\sim$ Napisz funkcję \type{iloraz}, która przyjmuje argumenty
...
$\sim$ Napisz funkcję, która porównuje dwie liczby. Jako argumenty
...
\subsection[prawdziwość-obiektów-funkcja-bool]{Prawdziwość obiektów,
funkcja \type{bool}}
...
obiekt, możemy posłużyć się funkcją zbudowaną \type{bool}. Przyjmuje ona
jeden argument - dowolny obiekt - i zwraca jego wartość logiczną:
\type{True} lub \type{False}.
...
$\sim$ Dla każdego z następujących typów odszukaj wartość, dla której
funkcja \type{bool} zwróci \type{True} i taką dla której zwróci
\type{False}: string, krotka, integer, float.

Ponieważ każdy obiekt można rozważać w kategorii
\quotation{prawdziwości}, każdym obiektem możemy posłużyć się w
instrukcji \type{if}:
...
$\sim$ Napisz funkcję, która jako argument przyjmie listę i zwróci
\type{True} jeżeli wszystkie elementy na tej liście są prawdziwe, albo
\type{False} jeżeli przynajmniej jeden element nie jest prawdziwy.

\subsection[pushpin-podsumowanie-11]{$\swarrow$ Podsumowanie}
...
\section[rozdział-13.-słowniki]{Rozdział 13. Słowniki}
...
$\sim$ Zobacz co się stanie jeżeli pobierzesz wartość dla klucza, który
nie istnieje w słowniku.

$\sim$ Napisz funkcję, która przyjmie dwa argumenty, listę słowników
...
$\sim$ Zobacz co się stanie jeżeli spróbujesz usunąć klucz, który nie
istnieje w słowniku.
...
$\sim$ Napisz funkcję, która przyjmuje jako argument słownik i
...
$\sim$ Napisz funkcję, która przyjmuje jako argument słownik i zwraca
...
$\sim$ Napisz funkcję, która przyjmie dwa argumenty, słownik oraz
...
$\sim$ Wybierz jedno z powyższych zastosowań słownika. Napisz funkcję
...
\subsection[pushpin-podsumowanie-12]{$\swarrow$ Podsumowanie}
...
\section[rozdział-14.-none]{Rozdział 14. \type{None}}

W tym rozdziale:

\startitemize[packed]
\item
  dowiesz się czym jest {\bf \type{None}}.
\stopitemize

\subsection[none]{\type{None}}

W Pythonie istnieje jedna szczególna wartość, która nie reprezentuje
żadnego dotąd poznanego typu: {\bf \type{None}}. Nie jest to liczba,
string, ani nic innego co poznasz ucząc się tego języka. \type{None} to
unikalna wartość, która reprezentuje osobny typ. Powstała po to, żeby
programista mógł zdefiniować \quotation{nic}. Po co? Okazuje się, że są
sytuacje, w których chcesz wprost zaznaczyć, że dana operacja nie
zwróciła żadnej istotnej informacji. W takich sytuacjach możesz użyć
wartości \type{None}. Zwróć uwagę, że sam fakt posłużenia się tą
wartością jest na tyle wyjątkowy, że sugeruje, że zaistniały jakieś
szczególne okoliczności.

Przykładem użycia \type{None} może być dzielenie przez 0. Jak wiemy,
taka operacja matematyczna jest niedozwolona. Teraz wyobraźmy sobie, że
piszemy funkję, która przyjmuje jako argumenty dwie liczby i zwraca
wynik dzielenia jednej przez drugą.
...
Co się stanie gdy argument \type{dzielnik} będzie równy \type{0}?
Otrzymamy błąd:
...
argumentu i w razie potrzeby zwrócić \type{None}, sygnalizując, że nie
możemy obliczyć wyniku:
...
Teraz z łatwością możemy rozpoznać kiedy funkcja napotkała
...
\subsection[domyślna-wartość-funkcji]{Domyślna wartość funkcji}

Napiszmy prostą funkcję, która przyjmuje argument i wypisuje go na
ekran:
...
\type{return}? Odpowiedź brzmi: funkcja zwróci \type{None}.
...
Tak samo dzieje się, jeżeli użyjemy instrukcji \type{return} nie podając
jej żadnej wartości:
...
Efekt będzie taki sam jak wtedy gdy nie użyjemy \type{return}:
...
przyjęli, że domyślnie funkcja zwraca \quotation{nic}, a wartość taka
będzie reprezentowana przez \type{None}.

\subsection[operator-is]{Operator \type{is}}

Skoro wiemy, że funkcje mogą zwracać \type{None}, to w jaki sposób z
tego skorzystać? Możemy na przykład sprawdzić zwróconą wartość w
instrukcji \type{if}:
...
W powyższym przykładzie funkcja \type{nazwa_uzytkownika} zwraca pełną
nazwę użytkownika na podstawie jego imienia, nazwiska i adresu email.
Jeżeli imię i nazwisko są dane, to zwracane jest pełne nazwisko, w
przyciwnym wypadku zwrócony zostanie tylko adres email. Sprawdzenie czy
pełne nazwisko jest podane zostało zapisane w instrukcji
\type{if pelne_nazwisko == None}.

Instrukcja \type{if zmienna == None} jest poprawna, jednak w ogólnym
przypadku może nie zawsze działać. Dzieje się tak, ponieważ tworząc
bardziej zaawansowane struktury danych w Pythonie (co nie będzie
omówione podczas tych warsztatów), możemy zmienić zachowanie porównania,
w efekcie wpływając na wynik zwracany przez operator \type{==}. Możemy
wybrnąć z tej sytuacji stosując zamiast tego {\bf operator \type{is}}:
...
Kiedy zatem używać \type{is}? Używaj tego operatora zawsze kiedy
porównujesz wartość do \type{None}. Póki co nie będzie to robiło
różnicy, ale dzięki temu nabędziesz dobrego nawyku, który w przyszłości
okaże się przydatny.

$\sim$ Napisz funkcję, która jako argument przyjmuje listę i zwróci
również listę, na której znajdą się wszystkie elementy z argumentu, z
wyjątkiem wartości równych \type{None}.

\subsection[pushpin-podsumowanie-13]{$\swarrow$ Podsumowanie}
...
\item
  poznaliśmy \type{None},
\item
  dowiedzieliśmy się, że porównując wartości do \type{None} należy
  stosować operator \type{is}.
\stopitemize

\section[rozdział-15.-pętla-while]{Rozdział 15. Pętla \type{while}}

W tym rozdziale:

\startitemize[packed]
\item
  nauczysz posługiwać się pętlą \type{while}.
\stopitemize

\subsection[pętla-while]{Pętla \type{while}}

Wiesz już czym jest pętla oraz znasz jedną z nich: \type{for}. Teraz
poznasz drugą, jednocześnie ostatnią jaka istnieje w Pythonie:
\type{while}. Jej struktura jest jeszcze prostsza od poprzedniej:
...
Definicję pętli zaczynamy słowem \type{while}, następnie definiujemy
warunek, a po drukropku, w kolejnych linijkach i po wcięciu, wypisujemy
instrukcje, które będą wykonywane tak długo jak warunek będzie
prawdziwy.

Zwróć uwagę, że jeżeli zdefiniujemy warunek, który zawsze będzie
prawdziwy, wtedy pętla będzie wykonywana w nieskończoność:
...
$\sim$ Napisz funkcję, która przyjmie listę jako argument i wypisze
wszystkie jej elementy przy użyciu pętli \type{while}.

\subsection[pushpin-podsumowanie-14]{$\swarrow$ Podsumowanie}

W tym rozdziale:

\startitemize[packed]
\item
  poznaliśmy pętlę \type{while}.
\stopitemize

\section[rozdział-16.-biblioteka-standardowa]{Rozdział 16. Biblioteka
standardowa}
...
\startitemize[packed]
\item
  dowiesz się czym jest {\bf biblioteka standardowa},
\item
  poznasz najważniejsze {\bf moduły} biblioteki standardowej.
\stopitemize

\subsection[biblioteka]{Biblioteka}

W programowaniu posługujemy się pojęciem {\bf biblioteka}, które oznacza
...
metematycznych (np. trygonometrycznych), do których możemy odwołać się w
naszym kodzie zamiast definiować je samodzielnie.

\subsection[moduły]{Moduły}

W Pythonie biblioteki programistyczne są dostępne poprzez {\bf moduły}.
Moduł to po prostu kod umieszczony na dysku, w miejscu w którym Python
może go odnaleźć. Może to być kod napisany w Pythonie, ale są sposoby na
pisanie modułów w innych językach programowania. Python znajdzie taki
kod jeżeli zostanie on umieszczony w jednym z określonych z góry
katalogów.

Każdy może napisać własny moduł do Pythona. W internecie znajdziemy
tysiące modułów, które różne osoby i firmy udostępniają. Możemy je
pobrać i używać ich do pisania programów. Możemy też tworzyć własne
moduły i dzielić się nimi z innymi użytkownikami.

Aby skorzystać z modułu musimy {\bf zaimportować} jego nazwę. Robimy to
instrukcją \type{import}. Gdy moduł jest już zaimportowany, możemy
używać funkcji i zmiennych, które zostały w nim zdefiniowane. Robimy to
wpisując nazwę modułu, a następnie, po kropce, nazwę obiektu.

W poniższym przykładzie importujemy moduł o nazwie \type{math} i
wywołujemy funkcję \type{sqrt}, która zwraca pierwiastek kwadratowy z
podanej liczby:
...
\subsection[biblioteka-standardowa-pythona]{Biblioteka standardowa
Pythona}

Poza modułami, które użytkownicy sami mogą pisać, istnieje zbiór
modułów, który zawsze jest dostępny w Pythonie. Ten zbiór nazywamy
{\bf biblioteką standardową}. Znajdziemy w nim dziesiątki modułów, a w
nich narzędzia do komunikacji sieciowej, obsługi różnych formatów
plików, obliczeń matematycznych etc.

Biblioteka standardowa jest obszernie udokumentowana na
\useURL[url2][https://docs.python.org/3/library/index.html][][oficjalnej
stronie Pythona]\from[url2].

\subsection[najważniejsze-moduły]{Najważniejsze moduły}
...
\section[math]{\useURL[url3][https://docs.python.org/3/library/math.html][][\type{math}]\from[url3]}

Moduł \type{math} zawiera kilkadziesiąt funkcji matematycznych, które
mogą okazać się pomocne przy prostych obliczeniach. Poniżej opisaliśmy
kilka z nich.

Funkcje \type{ceil} i \type{floor} zwracają wartości zaokrąglone
...
Funkcja \type{sqrt} zwraca pierwiastek kwadratowy z danej liczby:
...
\type{pi} oraz \type{e} reprezentują wartości stałych matematycznych:
...
$\sim$ Napisz funkcję, która zwróci wartość pola powierzchni koła o
zadanym promieniu (według wzoru \type{PI * r^2}, gdzie \type{r} to
promień).

\section[datetime]{\useURL[url4][https://docs.python.org/3/library/datetime.html][][\type{datetime}]\from[url4]}

Moduł \type{datetime} to podstawowe narzędzie do pracy z datami i
czasem. Zawiera obiekty \type{date}, \type{datetime} oraz
\type{timedelta}, które reprezentują odpowiednio datę, datę i czas oraz
różnicę w czasie.

Aby otrzymać dzisiejszą datę należy wywołać metodę \type{today} na
obiekcie \type{date}:
...
Otrzymany w ten sposób obiekt zawiera trzy {\bf atrybuty}: \type{year},
\type{month} oraz \type{day}, czyli odpowiednio rok, miesiąc i dzień.
...
Metoda \type{now} na obiekcie \type{datetime} zwróci nam datę i godzinę,
którą mamy w tej chwili. Poza atrybutami \type{year}, \type{month} oraz
\type{day} posiada także \type{hour}, \type{minute}, \type{second} i
\type{microsecond}, czyli godzinę, minutę, sekundę i mikrosekundę.
...
wywołać \type{date} lub \type{datetime} i podać kolejno \type{year},
\type{month}, \type{day}, \type{hour}, \type{minute}, \type{second},
\type{microsecond}:
...
$\sim$ Zobacz co się stanie jeżeli spróbujesz utworzyć obiekt
\type{date} z miesiącem spoza zakresu od 1 do 12 lub datę, która nie
istnieje, np. 31 kwietnia.

$\sim$ Zobacz co się stanie jeżeli spróbujesz utworzyć obiekt
\type{datetime} z godziną, minutą lub sekundą o wartosci spoza
dopuszczalnego zakresu (np. godzina 26).

Jeżeli chcemy zobaczyć różnicę w czasie między dwoma obiektami typu
\type{datetime}, możemy je po prostu odjąć od siebie:
...
W powyższym przykładzie otrzymaliśmy obiekt \type{timedelta}, który
posiada atrybut \type{seconds} o wartości równej liczbie sekund pomiędzy
\type{a} i \type{b}.

Jeżeli różnica jest większa niż jedna doba, to zostanie ona podzielona
na dwie wartości: \type{seconds} i \type{days}, czyli sekundy i dni.
...
Jeżeli do daty \type{c} dodamy jeden dzień i 79200 sekund, to otrzymamy
\type{d}.

$\sim$ Napisz funkcję, która przyjmuje dwie daty jako argumenty. Jeżeli
druga data jest mniejsza od pierwszej, funkcja powinna zwrócić
\type{None}. W przeciwnym wypadku funkcja powinna zwrócić liczbę sekund
...
\section[random]{\useURL[url5][https://docs.python.org/3/library/random.html][][\type{random}]\from[url5]}

Moduł \type{random} służy do wykonywania operacji, których wynik jest
losowy: generowania liczb losowych, losowego wybierania obiektów z
danego zakresu, itd.

Funkcja \type{randint} przyjmuje dwa integery jako argumenty i zwraca
losowo wybraną liczbę całkowitą, której wartość znajduje się pomiędzy
argumentami.
...
Funkcja \type{choice} przyjmuje jako argument dowolną sekwencję (listę,
krotkę, string) i zwraca losowo wybrany element:
...
$\sim$ Napisz funkcję, która przyjmie jako argument dowolną sekwencję i
zwróci {\em krotkę} z trzema losowo wybramymi z niej elementami.

\section[json]{\useURL[url6][https://docs.python.org/3/library/json.html][][\type{json}]\from[url6]}

Moduł \type{json} pozwala zapisywać obiekty Pythona (słowniki i listy)
jako string w formacie JSON ({\em JavaScript Object Notation}), który
jest powszechnie używany np. w serwisach internetowych do wymiany danych
między przeglądarką a serwerem.

Żeby zapisać obiekt do stringa należy wywołać funkcję~\type{dumps}:
...
Do zamiany stringa na słownik lub listę służy funkcja \type{loads}:
...
Oczywiście nie trzeba znać jej w całości żeby swobodnie programować w
Pythonie. Wiele z tych modułów ma bardzo specyficzne zastosowania i
większość programistów nigdy z nich nie korzysta. Warto natomiast
pamiętać, że jeżeli trafisz na jakiś nowy problem, to dobrze jest
najpierw przejrzeć bibliotekę standardową w poszukiwaniu modułu, który
może okazać się pomocny.

\subsection[pushpin-podsumowanie-15]{$\swarrow$ Podsumowanie}

W tym rozdziale:

\startitemize[packed]
\item
  dowiedzieliśmy się czym jest {\bf biblioteka standardowa} i jak z niej
  korzystać,
\item
  poznaliśmy moduły \type{math}, \type{datetime}, \type{random} i
  \type{json}.
\stopitemize
...
mają dla Ciebie jeszcze garść zadań. Możesz też skorzystać z okazji i
zadać mentorom tyle pytań na ile tylko starczy czasu - między innymi
dlatego zorganizowaliśmy te warsztaty!

\subsection[o-czym-nie-powiedzieliśmy]{O czym nie powiedzieliśmy}
...
interesujące, ale nie wiesz jak zacząć. Dlatego przygotowaliśmy dla
Ciebie listę zagadnień, którą możesz potraktować jako kontynuację tych
warsztatów. Nie jest to wyczerpująca lista tematów, ale na pewno pomoże
Ci jeszcze lepiej poznać Pythona. W każdym punkcie umieściliśmy odnośnik
do strony, która wyjaśnia dany temat.

\startitemize[packed]
\item
  \useURL[url7][https://docs.python.org/3.6/tutorial/errors.html][][Jeszcze
  więcej o wyjątkach i obsługiwaniu ich]\from[url7]
\item
  \useURL[url8][https://docs.python.org/3.6/tutorial/classes.html][][Klasy,
  czyli programowanie zorientowane obiektowo]\from[url8]
\item
  \useURL[url9][https://docs.python.org/3/glossary.html\#term-decorator][][Dekoratory]\from[url9]
\item
  \useURL[url10][https://docs.python.org/3/glossary.html\#index-17][][Generatory]\from[url10]
\item
  \useURL[url11][https://docs.python.org/3/tutorial/datastructures.html\#list-comprehensions][][Listy
  składane]\from[url11]
\item
  \useURL[url12][https://docs.python.org/3/tutorial/datastructures.html\#sets][][Zbiory]\from[url12]
\stopitemize

\subsection[na-koniec-dir]{Na koniec\ldots{} \type{dir}}

Kończąc pokażemy Ci jeszcze jedną, bardzo przydatną funkcję wbudowaną.
Nazywa się \type{dir}, przyjmuje jako argument dowolny obiekt i zwraca
listę nazw wszystkich metod i atrybutów tego obiektu:
...
\type{items} czy \type{values}, ale jak widzisz słowniki oferują
znacznie więcej. Zwróć uwagę na nazwy zaczynające i kończące się
podwójnym znakiem podkreślenia. W taki sposób są nazywane atrybuty i
metody, które nie są przeznaczone dla użytkowników obiektu. Zazwyczaj są
to rzeczy używane tylko wewnątrz danego obiektu. Oczywiście tak czy
inaczej można z nich korzystać - po prostu zazwyczaj nie będą dla nas
przydatne, więc warto zainteresować się pozostałymi nazwami.
...
polecamy - użyć funkcji \type{help}, która wyświetli dokumentację danego
obiektu:
...
Dziękujemy za udział w warsztatach! Jeszcze raz zachęcamy do rozmowy z
mentorami - chętnie odpowiedzą na wszystkie pytania i wyjaśnią
