...
\placefigure{The ZODB Ecosystem by Christopher
Lozinski}{\externalfigure[ZODBTitleGraphics.svg]}

\subsection[abstract]{ABSTRACT}

ZODB is an object-oriented database written in Python and optimized in
C. It is widely used in the US, but not as well known in Poland. This
talk introduces the database, presents important concepts, and reviews
the tools available.

\subsection[about-the-author]{About the Author}

Christopher Lozinski has been using the ZODB since 1999.\crlf
He is an MIT graduate and silicon valley expat who has been using
dynamically bound languages for 35 years. First it was Lisp, then
Objective-C, and now Python. On the ZODB, he built PythonLinks.info,
PolandTrade.info, PrivaCV.com, Zopache.com, and the recently
decommissioned SpecialtyJobMarkets.com. He is a dual Polish-US citizen,
and polyglot.
...
ZODB is an object-oriented databaase written in Python and optimized in
C. This article is a good representation of the talk I am giving at
Pycon Poland 2017.

\subsection[so-easy-to-use]{So Easy to use}

ZODB is really easy to use. Just subclass off of class
...
\section[atomicity]{Atomicity}

Atomicity requires that each transaction be \quotation{all or nothing}.
...
Durability Durability ensures that once a transaction has been
...
\subsection[canonical-urls]{Canonical URL's}
...
PythonLinks provides canonic urls. . Every node has a unique name which
can be accessed using a dictionary in the root node.\crlf
Attempts to create a new node with the same name, result in an integer
being appended to the new node name. Node, Node-1, Node-2, etc. When
objects are renamed, or deleted, the root dictionary is updated.
...
\item
  {\bf ZEO} is the basic client server version of the ZODB.\crlf
  ZEO clients share data from the ZEO server. Each participating Python
  application server includes a ZEO client. They all talk to the ZEO
  server. The ZEO server will then store the data in FileStorage and
  BlobStorage
\item
...
\item
  ...
  stores data as both Python pickles, as well as JSONB Postgres data
...
\subsection[zodb-tutorial]{ZODB Tutorial}
...
So which platform should you use with ZODB?\crlf
I am not an expert in all of these platforms, but I will give you my
best advice on when to use each of them.
...
it three different ways. You can choose from lots of different GUI and
other libraries.
...
problem to have. I can then optimize things. Above all else, I care
about speed and cost of development. Related to that is software
flexibility.

\section[zopache]{Zopache}

Sadly Grok lacks a TTW development environment, so I built one.
Zopache.com is a Javascript IDE which I built on top of the ZODB using
Grok. ZODB developers might call it a TTW development platform.

Basic Zopache objects include HTML, CSS, Javascript, Python Scripts,
Image, File and Jade.\crlf
HTML objects can be edited with either the WYSIWYG HTML editor, or the
technical Ace Editor. CSS Javascript and Python Scripts can also be
edited with the Ace Editor.
...
folders can be edited with the Ace and ck editors. Javascript Folders
allow one to build a well organized tree of Javascript code, but serve
it as a single minified file or as a human readable and searchable file,
which clickable links to the original javascript object. History and
Undo are supported.

Using Zopache, in your browser, you create a tree of folders, and
populate and edit those objects.\crlf
Every object has its own URL, and can be displayed or edited. These is
no need to know Unix. But you do have the option to grow the application
with real Python code.

\section[flask]{Flask}

Flask is a very lightweight framework which can be used with the ZODB.
Flask does routing, so one has to manually map from URL's to objects.
...
demo page. http://demo.substanced.net/ Clearly they are very conflicted
about minimal TTW development, let alone the idea of building a full
Javascript IDE on top of the ZODB.
...
way to do things.\crlf
I want a purist OO approach.\crlf
Like Ruby on Rails, I want an opinionated high level framework. Grok
gives me that.

Pyramid is focused on multiple deployments. So TTW makes no sense.\crlf
I am focused on building a single website, so storing stuff in the ZODB
is much more modern than storing it on a 40 year old filesystem.
...
I also use components. Pyramid only supports view components.\crlf
But the Zope Component Architecture is now available with PyAMS.\crlf
...
is also possible to add You Tube videos, blogs, product reviews, and
...
\subsection[more-information]{More Information}
