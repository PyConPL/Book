...
command:
\type{server $ wget -qO- https://raw.github.com/progrium/dokku/v0.3.26/bootstrap.sh | sudo DOKKU_TAG=v0.3.26 bash}
...
run the following:
\type{$ cat ~/.ssh/id_rsa.pub | ssh root@<machine-address> "sudo sshcommand acl-add dokku <your-app-name>"}
...
\type{local $ git remote add dokku dokku@<machine-address>:<your-app-name>}
Push application to Dokku:
\type{local $ git push dokku <branch-name>:master} Pay attention, that
dokku will deploy your application {\bf only} if you push to the master
branch!
...
\type{export BUILDPACK_URL=https://github.com/OShalakhin/heroku-buildpack-geodjango}
...
\type{cd /var/lib/dokku/plugins git clone https://github.com/jeffutter/dokku-postgresql-plugin postgresql dokku plugins-install}
Then we create a database for application:
\type{server $ dokku postgresql:create <your-app-name>} This command
automatically creates and wires a database to \type{<your-app-name>}
application. It also outputs an appropriate
...
\startitemize[n][stopper=.]
\item
  \useURL[url1][https://en.wikipedia.org/wiki/Platform_as_a_service][][PaaS
  Wiki]\from[url1]
\item
  \useURL[url2][http://progrium.viewdocs.io/dokku/][][Dokku
  Documentation]\from[url2]
\item
  \useURL[url3][http://deis.io/][][Deis]\from[url3]
\stopitemize
