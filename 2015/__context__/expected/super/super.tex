...
klasy, na której ten wrapper został założony. Po przejrzeniu tego
artykułu przez kolegę~Nihilifer'a{\em , okazało się że jest możliwość
zrobienia takiego dekoratora. Wystarczy użyć functools.wraps} jako
dekorator funkcji wrapper. Dzięki temu dekorator zwróci nam metodę,
którą udekorowaliśmy. Chwila przeszukiwania stackoverflow da nam
funkcję, która zwróci nam klasę w której dana metoda była zdefiniowana.*

``` import inspect from functools import wraps

def
get\letterunderscore{}class\letterunderscore{}that\letterunderscore{}defined\letterunderscore{}method(meth):
if inspect.ismethod(meth): for cls in
inspect.getmro(meth.{\bf self}.{\bf class}): if
cls.{\bf dict}.get(meth.{\bf name}) is meth: return cls meth =
meth.{\bf func} \# fallback to {\bf qualname} parsing if
inspect.isfunction(meth): module = inspect.getmodule(meth) data =
meth.{\bf qualname}.split('.', 1){[}0{]}.rsplit('.', 1){[}0{]} cls =
getattr(module, data) if isinstance(cls, type): return cls return None

def superme(fun): @wraps(fun) def wrapper(*args, *{\em kwargs): return
...
Two().method() ``` I to nam da taki wynik:
