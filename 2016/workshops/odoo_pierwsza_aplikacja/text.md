...
class Beer(models.Model):
    ...
    def _calc_spejson_rating(self):
        for piwo in self:
            piwo.spejson_rating = piwo.alc * piwo.volume / piwo.price if piwo.price > 0 else 999999
    ...
    price = fields.Float('Cena', required=True)
    rating = fields.Selection(RATINGS, string='Ocena')
    spejson_rating = fields.Float('Ocena Spejsona', compute='_calc_spejson_rating')
```
