...
{[}Mountepy{]} (https://github.com/butla/mountepy) library I made,
Pytest and Docker), how to enforce TDD (using multi-process code
coverage) and good code style (with automated Pylint checks) within a
team.
...
\placefigure{}{\externalfigure[images/tdd_cycle.png]}
...
\placefigure{}{\externalfigure[images/test_pyramid.png]}
...
\placefigure{}{\externalfigure[images/fixtures.svg]}
...
found in
\useURL[url1][https://github.com/butla/pydas][][PyDAS]\from[url1], which
was my guinea pig for microservice TDD experiments.
...
In PyDAS, I've used
\useURL[url2][https://tox.readthedocs.io][][Tox]\from[url2] to automate
test runs. Below is an abbreviated version of Tox configuration
(\type{tox.ini}).
...
\useURL[url3][http://swagger.io/][][Swagger]\from[url3] is an interface
definition language. It will serve us as a contract definition language
description.
...
\useURL[url4][https://github.com/Yelp/bravado][][Bravado]\from[url4] is
a library that can dynamically create client objects for a service based
on its Swagger contract. It can do contract tests by automatically
validating the types (schemas) of both parameters and the values
returned from services. Can verify HTTP status codes. Status code and
response schema combinations that don't exist in Swagger are treated as
invalid, e.g.~if we can return a Person object with code 200 and an
...
did for \useURL[url5][https://falconframework.org/][][Falcon]\from[url5]
with
\useURL[url6][https://github.com/butla/bravado-falcon][][bravado-falcon]\from[url6],
thanks to Bravado's extensibility. Such integration can probably be
easily developed for other web frameworks.
